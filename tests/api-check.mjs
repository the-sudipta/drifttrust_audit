...
function client(){const cookies=new Map();return async(path,body,expected=200,extra={})=>{
 const r=await fetch(base+path,{method:body===undefined?'GET':'POST',headers:{Cookie:[...cookies].map(([k,v])=>`${k}=${v}`).join('; '),...(body!==undefined?{'Content-Type':'application/json'}:{}),...extra},body:body===undefined?undefined:JSON.stringify(body)});
 for(const header of r.headers.getSetCookie()){const pair=header.split(';')[0],split=pair.indexOf('=');cookies.set(pair.slice(0,split),pair.slice(split+1));}
...
const first=await a('/api/replay',body);assert.equal(first.predictions.length,expected.length);
// Math transcendental functions can differ by a few ULPs across V8 builds.
const replayParityMaxError=Math.max(...first.predictions.map((p,i)=>Math.abs(p.attack_probability-expected[i])));
assert.ok(replayParityMaxError<1e-10,`Replay parity error ${replayParityMaxError}`);
...
const report={base,checked_at_utc:new Date().toISOString(),passed:true,checks:['health and database','exported-model inference parity','same-engine replay parity (1e-10 tolerance)','invalid input rejection','cross-origin write rejection','session isolation','idempotent retries','stale revision rejection','full replay','audit hash-chain integrity','unlabelled inference-only stream'],replay_parity_max_absolute_error:replayParityMaxError,replay_flows:sa.seen,actual_candidates:audit.records.length,accepted:sa.accepted,rejected:sa.rejected};
