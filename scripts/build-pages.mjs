...
for(const p of ['index.html','lab.html','guide.html','site.css','site.mjs','lab.mjs','explanations.mjs'])fs.copyFileSync(p,path.join(out,p));
fs.mkdirSync(path.join(out,'runtime'));for(const p of ['engine.mjs','browser-client.mjs','browser-worker.mjs'])fs.copyFileSync('runtime/'+p,path.join(out,'runtime',p));
